...
function PhaseIndicator({
  ...
  useEffect(() => {
    const handlePhaseUpdate = (phaseData: PhaseData) => {
      console.log('[PhaseIndicator] updatePhase received:', phaseData);
    ...
    const handlePhaseChangeMessage = (phaseData: PhaseData) => {
      console.log('[PhaseIndicator] phaseChangeMessage received:', phaseData);
  ...
  }, [socket, gameId, playerId, currentPhase, onPhaseChange]);

  const nextPhase = useCallback(() => {
    if (!isMyTurn || isAnimationActive) {
      console.log('[PhaseIndicator] nextPhase bloqué:', { isMyTurn, isAnimationActive });
      return;
    }
    ...
    switch (currentPhase) {
      ...
      case 'Battle':
        ...
        return; // Ne pas émettre updatePhase pour 'End'
      case 'End':
        newPhase = 'Standby';
        break;
    }
    ...
    if (newPhase !== currentPhase) {
      console.log('[PhaseIndicator] Émission updatePhase:', { gameId, newPhase, turn });
